...
public class Logger
{
    ...
    {
        string msg = CommonUtils.ConnectStrs(ZString.Format("{0:yyyy-MM-dd HH:mm:ss,fff  }", DateTime.Now), ConnectStrs(msgs));
        Debug.Log(msg);
    }
    ...
    public static void PrintColor(string color, params string[] msgs)
    {
        ...
        {
            sb.Append(CommonUtils.ConnectStrs(ZString.Format("{0:yyyy-MM-dd HH:mm:ss,fff  }", DateTime.Now)));
            sb.Append("<color=");
            sb.Append(color);
            sb.Append(">");
