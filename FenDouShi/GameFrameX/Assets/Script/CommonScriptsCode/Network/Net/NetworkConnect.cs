...
public class NetworkConnect
{
    ...
    public void Connect(string ip, int port, Action<ConnectError> connectCallback) {
        ...
        if (IsIP(m_ip)) {
            ...
            Logger.PrintLog(CommonUtils.ConnectStrs("m_socket.BeginConnect ", m_ip, ":", m_port.ToString()));
    ...
    private void ConnectCallback(IAsyncResult ar) {
        ...
        if (m_isConnected) {
            ThreadManager.RunMainThread(() => {
                Logger.PrintLog("NetworkConnect  Socket_Connect_Succeed");
    ...
    public void RecievePackage(uint protoID, byte[] buffer) {
        ThreadManager.RunMainThread(() => {
            if (protoID != 11006)
                Logger.PrintLog(CommonUtils.ConnectStrs("收到协议 ====> ", protoID.ToString()));
        });
        Logger.PrintLog(CommonUtils.ConnectStrs("收到协议 ====> ", protoID.ToString()));
    ...
    public bool ProcessSendPackage(Package package) {
        ...
        if (id != 11005)
            Logger.PrintLog(CommonUtils.ConnectStrs("发送协议 ====> ", id.ToString()));

        byte[] idByte = BitConverter.GetBytes(id);

        int length = 2;
        if (package.ByteArray == null) {
            Logger.PrintError(CommonUtils.ConnectStrs("协议内容为空 ====> ", id.ToString()));
            return false;
        }
        //长度加2
    ...
    public void DispatchSocketError() {
        ThreadManager.RunMainThread(() => {
            Logger.PrintLog(CommonUtils.ConnectStrs("Socket连接错误:", serverName));
            if (m_connectCallback != null)
