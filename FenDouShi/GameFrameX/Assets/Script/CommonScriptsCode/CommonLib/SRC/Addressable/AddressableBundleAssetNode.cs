...
public class AddressableBundleAssetNode : AssetNode
{
    ...
    //private void AsyncOperationHandle<T> assethandle;
    ...
    public AddressableBundleAssetNode(AssetType assetType, string packageName, string relativePath) : base(assetType, packageName, relativePath)
    {
       // Logger.PrintColor("blue", "生成AddressableBundleAssetNode assetType=" + assetType+" packageName =" + packageName+ " relativePath="+ relativePath);
    }
    public void LoadFromFileAsync()
    {
        ...
        switch (assetType)
        {
            ...
            case AssetType.Model:
                ...
                //  AsyncOperationHandle<GameObject> addressableBunlde= Addressables.LoadAssetAsync<GameObject>(new List<object> { addressStr, lable });
                boundleOper = Addressables.LoadAssetsAsync<GameObject>(new List<string> { addressStr, lable }, null, Addressables.MergeMode.Intersection);
               
                updateCompleteState();
                // addressableBunlde.Result
                break;
            case AssetType.Scene:
                Logger.PrintColor("yellow", "Addressables.LoadSceneAsync addressStr=" + addressStr);
                boundleOper = Addressables.LoadSceneAsync(addressStr, UnityEngine.SceneManagement.LoadSceneMode.Additive);
                updateCompleteState();
                break;
    ...
    private object getResult(AsyncOperationHandle handle)
    {
        ...
        else if (this.assetType== AssetType.Scene)
        {
        //    AsyncOperationHandle <SceneInstance> sceneHandle = boundleOper ;
            return boundleOper.Result;
        }
        return null;
    }
