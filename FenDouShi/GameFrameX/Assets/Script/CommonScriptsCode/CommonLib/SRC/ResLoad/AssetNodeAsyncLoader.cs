...
#if AssetBundleLoad
        ////老版本过时  AssetBundle加载方式
        //if (assetNode is AssetBundleAssetNode)//老版本过时  
        //{
            ...
            for (int i = 0; i < m_ProcessingRequest.Count; ++i)
            {
...
#if AssetBundleLoad
                //老版本 assetBoundle
                if (assetNode is AssetBundleAssetNode)//老版本 assetBoundle
                {
                    AssetBundleAssetNode assetBundleAssetNode = assetNode as AssetBundleAssetNode;
                ...
                if (assetNode is AddressableBundleAssetNode)
                {
                    ...
                    if (assetBundleAssetNode.operationHandle.IsDone)
                    {
                        ...
                        //{
                        //    // 等待资源加载完成
                        //    yield return CommonUtils.StartCoroutine(assetBundleAssetNode.OnAssetBundleLoadedAsync());
...
#if AssetBundleLoad
    /// <summary>
    /// //老版本 assetBoundle节点依赖的节点是否是已经下载完成的状态
    /// </summary>
