...
public class RefTypes : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("======4444 8.17 2£º00 init Start() UI=" + LayerMask.NameToLayer("UI"));
        Debug.Log("======4444 8.17 2£º00 init Start() TimeZone=" + TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)));
        ParticleSystem.Particle[] m_Particles = new ParticleSystem.Particle[10];
        Debug.Log("======4444 8.17 2£º00 init Start() ParticleSystem=" + m_Particles[0].position);
        CapsuleCollider target = this.gameObject.AddComponent<CapsuleCollider>();
        Debug.Log("======4444 8.17 2£º00 init Start() target=" + target); 

        Debug.Log("======4444 8.17 2£º00 init Start() ParticleSystem=" + target.bounds.Contains(Vector3.zero));

        Debug.Log("======4444 8.17 2£º00 init Start() ParticleSystem=" + target.bounds.Contains(Vector3.zero));
        ScriptableObject bj = new ScriptableObject();
        ScriptableObject obi2 = ScriptableObject.CreateInstance<ScriptableObject>();
        FlareLayer lay = this.gameObject.AddComponent<FlareLayer>();
