 ...
 //   private static readonly object _lock = new object();
        ...
        //    lock (_lock)
          //  {
                if (instance == null)
                {
                    ...
                    Debug.Log("==============single objs.count=" + objs.Length+ "typeof="+ typeof(T));
                    if (objs.Length > 0)
                    {
                        instance = ((T)objs[0]);
                        if (objs.Length > 1)
                        {
                            ...
                            {
                                Debug.LogError("[Singleton] " + typeof(T).Name + " should never be more than 1 in scene!");
                            }
                            return instance;
                        }
                    }
    ...
    void Awake()
    {
        if (instance == null) {
            ...
            Debug.LogError("SingleMonobehaviour name[" + this.gameObject.name + "] should not be one more");
            isInit = false;
            Destroy(instance);
        }
    }
