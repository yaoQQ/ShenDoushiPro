...
public class ProtoHandler
{
   ...
   public void Close()
    ...
    void ReceiveCallback(object sender, SocketAsyncEventArgs e)
    {
        if (e.SocketError == SocketError.Success)
        {
            Logger.PrintDebug("ReceiveCallback:" + e.SocketError);
            if (sender is Socket)
            {
            ...
            {
                //服务器要求断开
                PrintLog(Utility.Platform.ConnectStrs(m_connect.serverName, " 服务器要求断开"));
                m_connect.Close(false);
            ...
            {
                PrintError(Utility.Platform.ConnectStrs(m_connect.serverName, " 接收数据长度有问题"));
            }
        }
        ...
        {
            //连接已断开
            Logger.PrintColor("red",Utility.Platform.ConnectStrs(m_connect.serverName, " 连接已断开"));
        ...
        {
            PrintError(Utility.Platform.ConnectStrs(m_connect.serverName, " Disconnect:", e.SocketError.ToString()));
            m_connect.Close(false);
            if (!NetworkManager.Instance.isLoginServer(m_connect.serverName))
                m_connect.DispatchSocketError();
        }
    ...
    void HandleData(byte[] data, int offset, int validLen)
    {
        ...
        if (offset >= validLen)
        {
           // Logger.PrintColor("red", $"如果偏移量大于等于有效长度，直接返回offset={offset}>={validLen} ");
            return;
        }
        // 如果当前状态为头部
    ...
    void ReadHead(byte[] data, int offset, int validLen)
    {
        ...
        if (headOffset == headLength)
        {
            ...
            Logger.PrintColor("red", $"@@@@@读取数据体长度  数据长度={bodyLength}");
           ...
           //  int len = getByteLen(headBuffer);
            ...
            //short len6 = BitConverter.ToInt16(headBuffer, 0);
    ...
    void ReadBody(byte[] data, int offset, int validLen)
    {
        ...
        {
            // 解析协议ID和数据
            protoID = BitConverter.ToUInt32(GetProtoIDBuff(bodyBuffer), 0);
