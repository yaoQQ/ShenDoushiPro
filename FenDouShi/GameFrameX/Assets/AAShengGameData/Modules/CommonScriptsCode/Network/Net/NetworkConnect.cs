...
public class NetworkConnect
{
    ...
    public void Connect(string ip, int port, Action<ConnectError> connectCallback) {
        Logger.PrintDebug("begain=================NetworkConnect  Connect");
        m_ip = ip;
        m_port = port;
        ...
        if (IsIP(m_ip)) {
            ...
            Logger.PrintLog(Utility.Platform.ConnectStrs("m_socket.BeginConnect ", m_ip, ":", m_port.ToString()));
            try
            {
            ...
            Logger.PrintLog("m_socket.BeginConnect " + m_ip + ":" + m_port);
    ...
    private void ConnectCallback(IAsyncResult ar) {
        try {
            ...
            //在回调方法中，调用 IAsyncResult 参数的 AsyncState 方法可以获取尝试连接的 Socket。
            //获取了 Socket 之后，可以调用 EndConnect 方法成功地完成连接尝试。
            handler.EndConnect(ar);

            // Add connection verification here
            bool isReallyConnected = handler.Connected &&
                                  (handler.Poll(1000, SelectMode.SelectRead) && handler.Available == 0) == false;
        ...
        if (m_isConnected) {
            ThreadManager.RunMainThread(() => {
                Logger.PrintColor("yellow", "NetworkConnect Socket_Connect_Succeed!!!");
    ...
    public void RecievePackage(uint protoID, byte[] buffer) {
        Logger.PrintColor("yellow", $"ThreadManager.RunMainThread()");
        ThreadManager.RunMainThread(() =>
        {
            if (protoID != 101006)
                Logger.PrintLog(Utility.Platform.ConnectStrs("收到协议 ====> ", protoID.ToString()));
        });
    ...
    public bool ProcessSendPackage(Package package) {
        ...
        uint id = package.ProtoID;
        if (id != 101005)
            Logger.PrintLog(Utility.Platform.ConnectStrs("发送协议 ====> ", id.ToString()));

        byte[] idByte = BitConverter.GetBytes(id);

        int length = 4;
        if (package.ByteArray == null) {
            Logger.PrintError(Utility.Platform.ConnectStrs("协议内容为空 ====> ", id.ToString()));
            return false;
        }
        //长度加4
        length = package.ByteArray.Length + 4;//字节总长度

        byte[] lengthByte = BitConverter.GetBytes(length);

        List<byte> all = new List<byte>();

        all.AddRange(lengthByte);//总长度

        all.AddRange(idByte);//协议号

        if (package.ByteArray != null) {
            all.AddRange(package.ByteArray);//协议数据
        }
        byte[] info = all.ToArray();//总长度+协议号+协议数据
    ...
    public void Close(bool isInitiative) {
        ...
        m_isConnected = false;
        if (m_socket != null) {
            Logger.PrintColor("red","CLOSE Socket");
            this.isInitiativeClose = isInitiative;
            m_protoHandler.Close();
            m_protoHandler = null;
            m_socket.Close();
        }
    }
    ...
    public void DispatchSocketError() {
        ThreadManager.RunMainThread(() => {
            Logger.PrintLog(Utility.Platform.ConnectStrs("Socket连接错误:", serverName));
            if (m_connectCallback != null)
