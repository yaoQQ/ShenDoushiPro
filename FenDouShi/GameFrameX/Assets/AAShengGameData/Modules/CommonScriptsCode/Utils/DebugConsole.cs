...
public class DebugConsole : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Input.touchCount == 3 && !_toggleTouch)
        {
            _isOpen = !_isOpen;
            _toggleTouch = true;
        }
        else if (Input.touchCount < 3 && _toggleTouch)
        {
            _toggleTouch = false;
        }
    ...
    void DrawBottomControls()
    {
        ...
        if (GUI.Button(buttonsRect, "Send", btnStyle))
        {
            ...
            Debug.Log("Send string=" + _inputString);
            string typeId = "type ";
            if (_inputString.IndexOf(typeId) != -1) {
                string[] typeStr = _inputString.Split(' ');
       
            }
            if (_inputString.Equals("type00")) {
               
            }
            if (_inputString.Equals("q")|| _inputString.Equals("Q"))
            {
                IsOpen = false;
            }
