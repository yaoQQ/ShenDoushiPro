...
public class WWWManager : MonoBehaviour
{
    ...
    public void load(string url, Action<UnityWebRequest> succeedCallback = null, Action<string> errorCallback = null, Action timeoutCallback = null, Action<float> progressCallback = null, bool isShowRetryDialog = false, int autoRetryCount = 0)
    {
        WWWLoader loader;
        if (this.m_wwwLoaderDic.ContainsKey(url))
        {
            ...
            loader.succeedCallback = (Action<UnityWebRequest>) Delegate.Combine(loader.succeedCallback, succeedCallback);
