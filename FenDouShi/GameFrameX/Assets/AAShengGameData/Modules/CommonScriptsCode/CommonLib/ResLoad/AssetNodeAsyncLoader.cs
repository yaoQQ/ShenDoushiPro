...
public class AssetNodeAsyncLoader
{
    ...
    public static void Download(AssetNode assetNode)
    {
        ...
        {
            Logger.PrintError(Utility.Platform.ConnectStrs("已经加载过了：", assetNode.relativePath));
            return;
        }
        if (IsDownloadingOrWaiting(assetNode))
        {
            Logger.PrintDebug(Utility.Platform.ConnectStrs("已经在加载队列中了：", assetNode.relativePath));
            return;
        }
        ResLoadManager.PrintLoadLog(Utility.Platform.ConnectStrs("加入等待加载队列：", assetNode.relativePath));
    ...
    private static void AddRequestToWaitingList(AssetNode assetNode)
    {
        ...
        {
            Logger.PrintDebug("IsLoadSuccess|IsInWaitingList assetNode.relativePath=" + assetNode.relativePath);
    ...
    private static void StartUpdate()
    {
        if (m_IsUpdate)
            return;
        m_IsUpdate = true;
        Utility.Platform.StartCoroutine(Update());
    }
    ...
    private static IEnumerator Update()
    {
        while (m_IsUpdate)
        {
            ...
            for (int i = 0; i < m_ProcessingRequest.Count; ++i)
            {
                ...
                if (assetNode is AddressableBundleAssetNode)
                {
                    AddressableBundleAssetNode assetBundleAssetNode = assetNode as AddressableBundleAssetNode;
                    //AssetBundleCreateRequest createRequest = assetBundleAssetNode.createRequest;
              
                    if (assetBundleAssetNode.operationHandle.IsDone)
                    {
                        object ab = assetBundleAssetNode.operationHandle.Result;
                     
                        if (ab == null&& assetBundleAssetNode.mAssetLoadStatus == AssetLoadStatus.Failed)
                        {
                            string path = Utility.Platform.ConnectStrs(assetBundleAssetNode.packageName, assetBundleAssetNode.relativePath);
                        ...
                        //// 加载失败 ，重新加载一次 ，确认该资源是没有被加载过的
                        //if (assetBundleAssetNode.mAssetLoadStatus== AssetLoadStatus.None)
                        //{
                        //    // 等待资源加载完成
                        //    yield return Utility.Platform.StartCoroutine(assetBundleAssetNode.OnAssetBundleLoadedAsync());
                ...
                else if (assetNode is TextAssetNode)
                {
                    ...
                    UnityWebRequest www = textAssetNode.www;

                    if (www.isDone)
                    {
                        ResLoadManager.PrintLoadLog(Utility.Platform.ConnectStrs("加载完成：", textAssetNode.relativePath));
                        if (www.error != null)
                        {
            ...
            while (m_ProcessingRequest.Count < MAX_PROCESSING && index < m_WaitingRequest.Count)
            {
                ...
                {
                    AddressableBundleAssetNode assetBundleAssetNode = assetNode as AddressableBundleAssetNode;
                    if (!assetBundleAssetNode.IsAssetLoadSuccess)
                    {
