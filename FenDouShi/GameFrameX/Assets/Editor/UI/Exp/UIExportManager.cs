...
public static class UIExportManager
{
   ...
   // public static bool isAutoExport = false;
    public static void AutoExportAndroid() {
        UIExportManager.ExportAllUIRes_Android();

        //@@@@@@@@@@@@
        //EffectExportManager.ExpEffectRes_Android();
        //OneKeyExpAudio.ExportAudioRes_Android();

      //  isAutoExport = false;
    }

    [MenuItem("Tools/UI/Export/Export One Mid ")]
    public static void ExportOneUI_Mid()
    {
        if (Selection.activeGameObject != null && !Selection.activeGameObject.activeInHierarchy && Selection.activeGameObject.GetComponent<UIBaseWidget>() != null)
        {
            ...
            EditorUtility.DisplayDialog("导出", "导出单个中间键成功 packageName=" + packageName + " relativePath=" + relativePath, "确定");
        }
        else
    ...
    [MenuItem("Tools/UI/Export/Export All Mid")]
    ...
    [MenuItem("Tools/UI/Export/Bind One Mono %E")]
    ...
    public static void ExportAllUIRes(string expPath, BuildTarget buildTarget, string loginPath)
    {
        ...
        Debug.Log("<color=blue>ExportAllUIRes()  path=" + Application.dataPath + "/Project/" + projectName + "/ui" + "</color>");
        foreach (string dir in dirArr)
        {
            DirectoryInfo directory = new DirectoryInfo(dir);
            Debug.Log("<color=yellow>ExportAllUIRes()  dir=" + dir + "</color>");
        ...
        Debug.Log("<color=blue>ExportAllUIRes()  outputPath=" + outputPath+"</color>");
        if (!Directory.Exists(outputPath))
            Directory.CreateDirectory(outputPath);
        BuildPipeline.BuildAssetBundles(outputPath, ui_abbList.ToArray(), options, buildTarget);

       // if (!UIExportManager.isAutoExport) {
    ...
    [MenuItem("Tools/UI/Export/Del All Res")]
