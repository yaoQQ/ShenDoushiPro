...
public static class ExportMiddleware
{
    ...
    static string GetWidgetTypeName(WidgetType widgetType)
    {
        ...
        switch (widgetType)
        {
            ...
            case WidgetType.Dropdown:
                typeName = typeof(DropdownWidget).ToString();
                break;
            default:
    ...
    static void ExecuteExp(UIPrefabData data)
    {
        ...
        string fileName = Application.dataPath + "/Script/Core/UI/Middleware/Mid_" + className + ".cs";
        ...
        {
            Debug.LogError("目录被打开，不能生成新文件：" + fileName+ ex);
            return;
        }
    ...
    static void WriteItemClass(Dictionary<string, ExpCell> expCellDic, ref StringBuilder sb)
    {
        foreach (KeyValuePair<string, ExpCell> kvp in expCellDic)
        {
            ...
            sb.Append("\t\tpublic GameObject main;\n");
    ...
    static void WriteClassMainFun(UIPrefabData data, ref StringBuilder sb)
    {
        ...
        sb.Append("\n\tvoid "  + "Awake() \n\t{\n");
        sb.Append("\t\tGameObject go =  this.gameObject;\n");
        sb.Append("\t\tmain =  this.gameObject;\n");
    ...
    static void WriteDelReferenceFun(UIPrefabData data, ref StringBuilder sb)
    {
        ...
        sb.Append("#if TOOL\n");
        sb.Append("#else\n");
        sb.Append("\t\tif(main!=null) GameObject.Destroy(main);\n");
        //sb.Append("\t\tUILoadTool.Instance.DelUIReference(" + "\"" + data.name + "\"" + ",1);\n");
        sb.Append("#endif\n");
        sb.Append("\t}\n");
    }
    ...
    static void Lua_ExecuteExp(UIPrefabData data)
    {
        ...
        string fileName = Application.dataPath + "/.." + "/res/lua/" + data.packName + "/mid/" + data.relativePath + className + ".lua";
        Debug.Log("fileName path=" + fileName);
        FileInfo fi = new FileInfo(fileName);
        var di = fi.Directory;
        if (!di.Exists)
            di.Create();
