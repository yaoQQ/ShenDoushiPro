...
public class BaseView:IBaseView
{
    ...
    public UIViewType getViewType()
    {
        ...
        {
            Logger.PrintError(this.viewName, " 没有设置 viewType");
        }
    ...
    public UIViewEnum getViewEnum()
    {
        if (this.viewEnum == UIViewEnum.None) {
            Logger.PrintError(this.viewName, " 没有设置 viewEnum");
    ...
    private void load()
    {
        ...
        for(int i = 0; i< this.loadOrders.Count; i++)
        {
            ...
            {
                Logger.PrintError(this.viewName, " loadOrders 加载列表格式错误");
                return;
            }
            //test@@@
    ...
    {
        Logger.PrintColor("blue", "executeLoadUIEnd uiName=" + uiName);
        this.loadedNum = this.loadedNum + 1;
        this.loadedNames.Add(uiName);
        Logger.PrintColor("blue", "executeLoadUIEnd gameObject=" + obj);
    ...
    {
        Logger.PrintWarning(this.viewName, " -- ", uiName, " 需重定义onLoadUIEnd");
    }
    ...
    {
        Logger.PrintWarning(this.viewName, " -- ", " 需重定义onShowHandler");
    }
    //--加载结束
    ...
    protected void BindMonoTable(GameObject gameObject, IMiddleware mid)
    {
        ...
        if (gameObject == null)
        {
            Logger.PrintError(this.viewName + "绑定Mono界面为空");
            return;
        }
        this.main_mid = mid;
        //test@@@@@  绑定
      ...
      //  panel.UIViewEnum = (int)this.viewEnum;
       ...
       // Logger.PrintDebug("=======================this:onLoadUIEnd================panel.UIViewEnum =" + (panel.UIViewEnum));
