...
public class UIViewManager : Singleton<UIViewManager>
{
    ...
    public GameObject CreateUILayerPanel(int layerIndex, string name)
    {
        ...
        Logger.PrintColor("red", "@@@@@@@@@@@@@@@CreateUILayerPanel Over");
        return uiLayerPanel;
    }
    public GameObject getCanvasByType(UIViewType viewType)
    {
        ...
        if (m_UILayerPanelDict.ContainsKey(index)){
            return m_UILayerPanelDict[index];
        }
        return null;
    }
    ...
    /// 注册界面
    /// </summary>
    public void RegisterView(string packageName, BaseView view)
    {
        ...
        {
            Logger.PrintError("m_UIViewDict.ContainsKey() viewEnum="+ viewEnum);
            return;
        }
        
        int layerIndex = (int)view.getViewType();
        if (!m_UILayerPanelDict.ContainsKey(layerIndex))
        {
    ...
    /// 打开界面
    /// </summary>
    public void Open(UIViewEnum viewEnum , System.Object msg = null, Action openCallback = null, bool isPush = true)
    {
        ...
        if (uiView == null)
        {
            Debug.Log(viewEnum.ToString() + " 窗口没有注册");
            return;
        }
    ...
    /// 关闭界面
    /// </summary>
    public void Close(UIViewEnum viewEnum, bool isDel = true, bool isAnim = true)
    {
        UIView uiView;
        m_UIViewDict.TryGetValue(viewEnum, out uiView);
        if (uiView == null)
        ...
        if (uiView.uiView.getIsStackView())
        {
            ...
            if (stackTop != null && (UIViewEnum)stackTop.viewEnum == viewEnum)
            {
                ...
                if (stackTop != null)
                {
                    Open((UIViewEnum)stackTop.viewEnum, stackTop.msg, stackTop.openCallback);
                    return;
                }
            }
        }
    ...
    public void CloseAllViewAndRevertStack()
    {
        ...
        OpenViewInfo stackTop = UIViewStack.GetStackTop();
        Open((UIViewEnum)stackTop.viewEnum, stackTop.msg, stackTop.openCallback, false);
    }
    ...
    public void ZoomAllView(Vector2 pivotV2, Vector3 value)
    {
        foreach (UIView uiView in m_UIViewDict.Values)
        {
            if (uiView.uiView.getViewType() != UIViewType.Alert_box)
            {
                if (uiView.uiView.getIsLoaded())
                {
                    RectTransform rt = uiView.uiView.getViewGO().GetComponent<RectTransform>();
                    rt.pivot = pivotV2;
                    rt.localScale = value;
                }
   ...
   // [BlackList]
    ...
    public void DestroyView(UIViewEnum viewEnum)
    {
        ...
        {
           GameObject go = baseView.getViewGO();
...
#region android设备返回
    ...
    /// </summary>
    private BaseView getCurrPopView() {
        ...
        for (int i = arr.Length - 1; i >= 0; i--) {
            ...
            //UIViewEnum viewEnum = (UIViewEnum)panel.UIViewEnum;
            int loginScene = 1001; //UIViewEnum.LoginView
            if ((int)viewEnum <= loginScene) {//登入界面时不炒作
                return null;
            }
            if (!m_UIViewDict.ContainsKey(viewEnum)) {
            ...
            BaseView view = m_UIViewDict[viewEnum].uiView;
            return view;
        }
    ...
    private returnTypeEnum GetDeviceReturnType(BaseView view) {
        UIViewType layerIndex = view.getViewType();
        if (!m_UILayerPanelDict.ContainsKey((int)layerIndex)) {
        ...
        UIViewType viewType = layerIndex;
        //判断层级对应的 返回状态
        ...
        if (viewType == UIViewType.Platform_Second_View) {
          //  Debug.LogFormat("Platform_Second_View");
            if (view.getIsStackView()) {//堆栈界面
                if (UIViewStack.IsStackButtomView((int)view.getViewEnum())) {//最底层界面
                    return returnTypeEnum.ExistApp;
                }
    ...
    public void CloseStackTopView() {
        BaseView luaView = getCurrPopView();
        if (luaView == null) {
            return;
        }
