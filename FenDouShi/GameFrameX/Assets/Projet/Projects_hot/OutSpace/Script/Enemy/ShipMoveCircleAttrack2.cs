...
public class ShipMoveCircleAttrack2 : Enemy
{
    ...
    {
        base.Start();
        targetPos = OutSpaceCameraManager.Instance.Player.position + OutSpaceCameraManager.Instance.Player.forward;
        LookAtPos(targetPos);
    }
    ...
    public override void attackPlayer()
    {
        ...
        if (shipAttrackType == EnemyAttrackShipType.FarAttack)
        {
            ...
            float angel = MyUtils.Angle(OutSpaceCameraManager.Instance.MainCamera.transform.position, this.transform.position, this.transform.forward);
            if (angel < 150)//对着才射击
            {
                return;
            }
    ...
    private void fly_begin()
    {
        ...
        Vector3 targetPos = OutSpaceCameraManager.Instance.Player.localPosition;
        Vector3 targetForwardVector = OutSpaceCameraManager.Instance.Player.forward;
        Vector3 targetRightVector = OutSpaceCameraManager.Instance.Player.right;
    ...
    private void CirclePlayer()
    {
        ...
        Vector3 pos2 = pos1 + vectorFromSelfToTarget * attrackDistance;
        //随机往左还是右偏移
        Vector3 flyPos3OffsetRight = UnityEngine.Random.Range(0, 10) > 5 ? -targetRightVector : targetRightVector;
        Vector3 pos3 = targetPos + flyPos3OffsetRight * attrackDistance * 0.5f;
        float pos3OffsetY = UnityEngine.Random.Range(0, attrackDistance * 0.5f);
        pos3.y += pos3OffsetY;

        Vector3 pos4 = targetPos + vectorFromSelfToTarget * (dis + attrackDistance * 1.2f);
