...
public class DisturbGun : PlayerGun
{
    ...
    {
        base.Update();
        if (startInput&&startInput.isSkillDisturbGun)
        {
            BtnPressFun();
        }
    }
    ...
    //从缓存的无用子弹中获取子弹
    private GameObject getSkillBullet()
    {
        if (DefendSkill != null)
        {
        ...
        GameObject obj = createSkillBullet();
        return obj;
    }

    //创建子弹
    private GameObject createSkillBullet()
    {
