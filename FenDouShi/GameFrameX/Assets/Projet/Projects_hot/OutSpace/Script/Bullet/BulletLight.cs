...
public class BulletLight : Bullet
{
    ...
    {
        base.Active();
        timeLife = gunInfo.getAttriCoolTime;
        isHit = false;
        currDistace = 1;
    }
    public override void SetBulletInfo(GunInfo gunInfo)
    {
        base.SetBulletInfo(gunInfo);
        maxDistace = gunInfo.getAttriShootDistance;
        timeLife = gunInfo.getAttriCoolTime;//打击时间间隔
    }
    ...
    public void OnTriggerStay(Collider other)
    {
      //  Debug.Log("OnTriggerStay");
        if (other.gameObject.layer==this.shootTargetLay)
        {
            isHit = false;
            return;
        }
        ...
        if (timeLife <= 0)//一次打击间隔
        {
          ...
          //  Debug.Log("hitObjs length=" + hitObjs.Length);
            for (int i = 0; i < hitObjs.Length; i++)
            {
                ...
                if (hit.collider.gameObject.layer!=this.shootTargetLay)//可能其他碰撞体，是敌方类型执行
                {
                    continue;
                }
