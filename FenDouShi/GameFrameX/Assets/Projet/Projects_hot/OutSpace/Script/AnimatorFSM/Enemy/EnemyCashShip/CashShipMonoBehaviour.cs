...
public class CashShipMonoBehaviour : EnemyBaseSMF
{
    ...
    private void OnEnable()
    {
        ...
        if (_animator)
        {
            SceneLinkedSMB<CashShipMonoBehaviour>.Initialise(_animator, this);
        }
    ...
    public void initAnimator()
    {
        ...
        float distance = Vector3.Distance(catchTransform.position, this.targetPos);
        _animator.SetFloat(hashDistanceToPos, distance);
        _animator.SetFloat(hashTargetPos, this.targetPos.z);

        LookAtRotate();
        float angel = Quaternion.Angle(this.targetRotation, catchTransform.rotation);
    ...
    public override void initPaths()
    {
        ...
        Vector3 iniMovePos = catchTransform.position + Vector3.left * Random.Range(-1, 1);
        Vector3 iniMovePos2 = catchTransform.position + Vector3.right * Random.Range(-1, 1);
    ...
    public void MoveToPos()
    {
        ...
        catchTransform.position = Vector3.MoveTowards(catchTransform.position, this.targetPos, this.currSpeed * Time.deltaTime);
    ...
    public void UpdateRotate()
    {
        ...
        {
            catchTransform.rotation = Quaternion.Lerp(catchTransform.rotation, this.targetRotation, this.RotateSpeed * Time.deltaTime);
        }
    }
    //是否旋转到目标位置的角度
    ...
    public void musicDoFun(float averMusic)
    {
        if (particleMusic)
        {
            ...
            {
                particleMusic.Emit(30);
            }
            else
            {
                particleMusic.Emit(10);
            }
        }
    }
