...
namespace UIWidget
{
    ...
    public partial class BannerWidget : IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        new private RectTransform rectTransform
        {
            get
            {
                if (m_Rect == null)
        ...
        public void SetBannerData<T>(List<T> p_dataList, int dataIndex, Action<GameObject, object, int> p_onUpdateCellData)
        {
            if (p_dataList == null)
            {
            ...
            m_dataList = toObjectList<T>(p_dataList);
        ...
        private List<object> toObjectList<T>(List<T> arr)
        {
            List<object> objArr = new List<object>();
          
            for(int i=0;i< arr.Count; i++)
            {
                objArr.Add((object)arr[i]);
             }
        ...
        public void ContentAnimaTo(float scale, Vector2 _position, Camera _camera)
        {
            if (m_Content == null)
                return;
            m_Content.localScale = new Vector3(scale, scale, 1);
            var local = UIExEventTool.ScreenToTargetLocal(m_Content.gameObject, viewRect.gameObject, _position, _camera); // 点击点的屏幕坐标相对于content局部坐标系的坐标
    ...
    {
        public bool AddPinchEventListener(UIEvent eventType, Action<PinchEventData> onEventHandler)
        {
            bool sign = true;
            switch (eventType)
            {
                case UIEvent.PinchIn:
                    UIExPinchListener.Get(gameObject).onPinchInHandler = onEventHandler;
                    break;
                case UIEvent.PinchOut:
                    UIExPinchListener.Get(gameObject).onPinchOutHandler = onEventHandler;
                    break;
                default:
                    break;
            }
        ...
        public bool RemovePinchEventListener(UIEvent eventType, Action<GameObject> onEventHandler)
        {
            bool sign = true;
            switch (eventType)
            {
                case UIEvent.PinchIn:
                    UIExPinchListener.Get(gameObject).onPinchInHandler = null;
                    break;
                case UIEvent.PinchOut:
                    UIExPinchListener.Get(gameObject).onPinchOutHandler = null;
                    break;
                default:
                    break;
            }
