...
public class ActorPointTools5 : MonoBehaviour
{
    ...
    public Vector3 column_mid = new Vector3();
    ...
    public void LogicPoint()
    {
        ...
        actorPoints[2].localPosition = new Vector3(-column_mid.x - row_1.x, column_mid.y + row_1.y, column_mid.z + row_1.z);
        actorPoints[3].localPosition = new Vector3(-column_mid.x - row_2.x, column_mid.y + row_2.y, column_mid.z + row_2.z);

        actorPoints[4].localPosition = new Vector3(-column_outside.x - row_1.x, column_outside.y + row_1.y, column_outside.z + row_1.z);
        actorPoints[5].localPosition = new Vector3(-column_outside.x - row_2.x, column_outside.y + row_2.y, column_outside.z + row_2.z);
        ...
        actorPoints[8].localPosition = new Vector3(column_mid.x + row_1.x, column_mid.y + row_1.y, column_mid.z + row_1.z);
        actorPoints[9].localPosition = new Vector3(column_mid.x + row_2.x, column_mid.y + row_2.y, column_mid.z + row_2.z);

        actorPoints[10].localPosition = new Vector3(column_outside.x + row_1.x, column_outside.y + row_1.y, column_outside.z + row_1.z);
        actorPoints[11].localPosition = new Vector3(column_outside.x + row_2.x, column_outside.y + row_2.y, column_outside.z + row_2.z);
    ...
    void OnDrawGizmos()
    {
        ...
        DrawLine(4, 5);   // 点1-点8 (左内-左中)
        DrawLine(2, 3);    // 点8-点4 (左中-左外)
        DrawLine(0, 1);    // 点2-点9
        DrawLine(6, 7);    // 点9-点5
        DrawLine(8, 9);   // 点1-点8 (左内-左中)
        DrawLine(10, 11);    // 点8-点4 (左中-左外)


        // 绘制排连接线 (绿色)
        var list1 = new int[] { 5, 3, 1, 7, 9, 11 };
        var list2 = new int[] { 6, 4, 2, 8, 10, 12 };
        Gizmos.color = Color.green;
        for (int i = 0; i < list1.Length - 1; i++)
        {
            DrawLine(list1[i] - 1, list1[i + 1] - 1);
        }

        for (int i = 0; i < list2.Length - 1; i++)
        {
            DrawLine(list2[i] - 1, list2[i + 1] - 1);
        }
    }
