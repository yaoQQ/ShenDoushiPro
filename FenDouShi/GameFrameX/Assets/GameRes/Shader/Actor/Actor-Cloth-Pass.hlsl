#ifndef ACTOR_CLOTH_PASS_INCLUDE
#define ACTOR_CLOTH_PASS_INCLUDE
...
void InitializeSurfaceData(float2 uv, FRONT_FACE_TYPE cullFace, out CustomSurfaceData outSurfaceData)
{
    outSurfaceData = (CustomSurfaceData)0;

    half4 diffuseAlpha = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv);
    half4 backDiffuseAlpha = SAMPLE_TEXTURE2D(_BackMap, sampler_BackMap, uv);

    diffuseAlpha = IS_FRONT_VFACE(cullFace, diffuseAlpha, backDiffuseAlpha);
...
// 计算MatCap UV
half2 CalculateMatCapUV(half3 normalWS, half3 viewDirWS)
{
    half3 r = reflect(-viewDirWS, normalWS);
    half m = 2.0 * sqrt(r.x * r.x + r.y * r.y + (r.z + 1.0) * (r.z + 1.0));
    return r.xy / m + 0.5;
}
half2 CalculateMatCapUV_URP(half3 normalWS, half3 positionWS)
{
    // 使用URP接口转换到视图空间
    half3 viewNormal = TransformWorldToViewNormal(normalWS);
    half3 viewPos = TransformWorldToView(positionWS);
    half3 viewDirVS = normalize(viewPos);
    
    // 计算反射向量
    half reflectDot = dot(viewDirVS, viewNormal);
    reflectDot = reflectDot + reflectDot;
    half3 reflectDirVS = viewNormal * (-reflectDot) + viewDirVS;
    
    // 计算MatCap UV
    half reflectDist = dot(reflectDirVS.xy, reflectDirVS.xy);
    half reflectZ = viewDirVS.z + 1.0;
    reflectDist = reflectDist + reflectZ * reflectZ;
    reflectDist = sqrt(reflectDist);
    reflectDist = reflectDist + reflectDist;
    
    half2 matcapUV = reflectDirVS.xy / reflectDist;
    return matcapUV + 0.5;
}

half4 Fragment(Varyings input, FRONT_FACE_TYPE cullFace : FRONT_FACE_SEMANTIC) : SV_Target
{
    half4 color = 0;
    float2 uv = input.uv;
    
    CustomSurfaceData surfaceData;
    InitializeSurfaceData(uv, cullFace, surfaceData);
